      ...
      <nav className={`bg-[#f2f2f2] w-[240px] min-h-screen h-full z-20 flex flex-col fixed top-0 transition-transform duration-300 ${sidebar ? 'translate-x-0' : '-translate-x-full'} shadow-md navbar-container lg:w-[260px]`}>
        ...
        <div className="flex items-center px-4 py-6 gap-3 bg-blue-50">
          ...
          <h1 className="text-2xl font-bold text-white">
            {loadData.role === 'superuser' && <Link to="/admin/dashboard" className="no-underline text-black">YIT PLACEMENT</Link>}
            {loadData.role === 'management_admin' && <Link to="/management/dashboard" className="no-underline text-black">YIT PLACEMENT</Link>}
            {loadData.role === 'tpo_admin' && <Link to="/tpo/dashboard" className="no-underline text-black">YIT PLACEMENT</Link>}
            {loadData.role === 'student' && <Link to="/student/dashboard" className="no-underline text-black">YIT PLACEMENT</Link>}
          </h1>
        </div>
